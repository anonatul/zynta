...
const createSellerProduct = async (req, res) => {
  try {
    const { title, description, price, category_id, stock, image_url } = req.body;
    if (!title || !price) {
      return res.status(400).json({ message: 'Title and price are required' });
    }
    const result = await query(
      `INSERT INTO products (seller_id, title, description, price, category_id, stock, image_url)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`,
      [req.user.id, title, description, price, category_id, stock, image_url]
    );
...
const updateSellerProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, price, category_id, stock, image_url, active } = req.body;
    const result = await query(
      `UPDATE products 
       SET title = COALESCE($1, title), description = COALESCE($2, description),
           price = COALESCE($3, price), category_id = COALESCE($4, category_id),
           stock = COALESCE($5, stock), image_url = COALESCE($6, image_url),
           active = COALESCE($7, active), updated_at = NOW()
       WHERE id = $8 AND seller_id = $9
       RETURNING *`,
      [title, description, price, category_id, stock, image_url, active, id, req.user.id]
    );
